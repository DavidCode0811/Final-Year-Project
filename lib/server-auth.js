...
export function createAuthenticatedRouteClient(accessToken) {
...
async function updateExistingProfile(client, profile, authUser) {
  const nextName = profile.name || getProfileName(authUser);
  const nextEmail = profile.email || authUser.email;
  const nextRole = profile.role || normalizeRole(getMetadataString(authUser, 'role'));
  const updates = {};
  ...
  if (!profile.password) {
    updates.password = SUPABASE_AUTH_PLACEHOLDER;
  }
