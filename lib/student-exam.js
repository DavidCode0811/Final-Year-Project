...
export function decodeAppToken(token) {
  if (!token) {
    throw new Error('You need to be signed in to access this exam.');
  }
...
export async function fetchStudentResponse(examId, studentId) {
  const { data, error } = await supabase
    .from('responses')
    .select('id')
    .eq('exam_id', examId)
...
export async function submitStudentExam({
  ...
  if (attemptId) {
    const nextStatus = submissionType === 'auto' ? 'auto_submitted' : 'submitted';
    ...
    if (error) {
      throw new Error(error.message || 'Failed to finalize your exam attempt.');
    }
  }
